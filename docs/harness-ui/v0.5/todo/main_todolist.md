...
## 마일스톤 개요 & 순서
- [ ] **M1 기반** — 스캐폴딩·runtime detect·inventory·agents/skills API·runs reader. (A1,A3api,A5be)
- [ ] **M2 Supervisor 코어** — spawn(로그파일·fd close)·서명 레지스트리·heartbeat·구조화로그 커서·events 멱등·원자쓰기. (A6be,A24,A25)
- [ ] **M3 OS 어댑터** — POSIX pgroup + Windows detached pid-tree(taskkill)·identity(shell)·reconcile 3-OS CI. (A18,A19,A20,A21,A21b,A22)
- [ ] **M4 화면+보안** — 6화면·drift·Ops·artifact·token·보안·**Overview 상태통계(A35-38)**·**배선/UX 보강(A39-A46: settings·state-stats·bootstrap·필드배선·drift-plan·artifact·UX표준)**. (A2-A5,A7,A8,A8b,A9a,A10-A14,A23,A26-A29,A35-A46)
- [ ] **M5 실행 인증** — dry-run·실 스모크(opt-in) 3-OS. (A6,A9b,A15,A16,A17,A17w)
- [ ] **M6 런처 + Codex toml** — 첫 실행 bootstrap(경화)·`.codex/agents` 동기화 계획. (A30,A30w,A31,A32,A33,A34) *(런처 R7 인증 후 착수)*
...
## 다음 단계 참조
- 착수 순서: 교차 관심사(schema·state_home·어댑터 IF) 선확립 → M1. 각 마일스톤 게이트 통과 후 다음.
- 런처(M6)는 설계 R7 인증 후. 그 전까지 M1-M5 진행 가능.
- 상세 단계별 계획: `M1_기반.md` ~ `M6_런처.md`.
