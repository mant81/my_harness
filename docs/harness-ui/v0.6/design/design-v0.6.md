...
- **판정:** v0.6 **비목표**(§5). 재검토 시 v0.7에서 별도 설계(PTY 컨테인먼트·WebSocket 인증·resume 실CLI 스모크 선행). 구 초안의 스키마 델타(conversationId/parentRunId/turnIndex/sessionId)는 **채택 보류** — 단, `manifest.agent`(F2)·events `usage`(F6)만 v0.6에 유지(대화와 무관·관측에 유용).
...
- **이름 열거 → FS시간 정렬 → 상위 N 내용 read(정렬 정합·runId 형식 무의존 — R3-#1·R4-#1):** `readdir`는 **엔트리 순서 미보장**이고 **`runId: z.string()`은 시간순 접두를 강제 안 함**(CLI/미래가 UUID·`run-1`/`run-10` 생성 가능 → 사전식 정렬 무작위). 정정:
  ...
  3. **상위 `MAX_RUNS_SCAN`개만 내용 바운드 read:** 정렬 상위 N에 대해서만 status·manifest를 `MAX_JSON_BYTES` 바운드 read(현 `readJsonSafe` 전체 read를 `readCapped`식 fd·fstat·상한 read로 교체). 손상/미검증 = `valid:false` skip(A5be). **OOM 방어 = 내용 read 상한(N × MAX_JSON_BYTES).**
...
- **삭제·미존재 에이전트(TOCTOU):** template 조회↔제출 사이 정의 삭제 시 제출은 **사용자 편집 domain으로 정상 진행**(실행은 정의 비의존). `manifest.agent`는 **형식검증(SAFE_SEGMENT)만·존재보장 안 함**(귀속 태그·경로 조립 아님·메모리 필터). 무해.
...
### F3.7 공유 config 스키마·원자 RMW (F3+F7 결합 파괴 정정 — R3-#3)
F3(`projectRoot`)·F7(`definitionEditEnabled`)이 **단일 `<state_home>/config.json`을 공유**하므로 결합 파괴(필드 clobber·부팅 폐기·lost update)를 다음으로 차단:
```ts
// R4-#4: 전체객체 strict 금지(한 필드 깨져도 전체 실패→전 필드 기본값→게이트 소거). 봉투 파싱 후 필드별 독립 검증·복구.
function loadConfig(raw: unknown): { schemaVersion: 1; projectRoot: string | null; definitionEditEnabled: boolean } {
  const env = (typeof raw === "object" && raw) ? raw as Record<string, unknown> : {};
  const schemaVersion = env.schemaVersion === "1" ? 1 : 1;                 // 봉투(미래 마이그레이션 훅)
  // 필드별 독립 파싱 — 한 필드 실패가 다른 필드를 오염시키지 않음.
  const pr = z.string().safeParse(env.projectRoot);
  const projectRoot = pr.success ? pr.data : null;                        // 무효/부재→null(부팅 시 D1-D7 재검증)
  const de = z.boolean().safeParse(env.definitionEditEnabled);
  const definitionEditEnabled = de.success ? de.data : false;            // 무효/부재/손상→false(fail-closed·독립 보존)
  return { schemaVersion, projectRoot, definitionEditEnabled };
}
```
- **원자 read-modify-validate-write(필드별·전 필드 보존):** 모든 config 쓰기(F3·F7)는 **read → `loadConfig`(필드별 복구) → 해당 필드만 수정 → 전 필드 재직렬화 → `writeJsonAtomic`**. 한 필드 쓰기·한 필드 손상이 다른 필드를 clobber/소거하지 않음(R3-#3a·R4-#4).
- **동시쓰기 직렬화:** config 쓰기는 **in-process 뮤텍스**(ingest `locks` 패턴)로 직렬화 → RMW lost-update 차단(R3-#3c·로컬 단일서버 전제).
- **부팅 필드별 검증·복구(R4-#4):** `loadConfig`로 봉투 파싱 후 **필드 독립 검증** — `projectRoot`는 추가로 D1-D7 재검증(실패 시 그 **값만** null·폴백)·`definitionEditEnabled`는 boolean 자체 검증(부재/손상→false). **projectRoot 손상이 definitionEditEnabled를 소거하지 않음**(R3-#3b 취지 관철). 전체객체 strict Zod 파싱 **금지**.
...
- **API:** `GET /api/evals`(loop 목록·최근 요약)·`GET /api/evals/:loop`(**신뢰 rollup 추세·R3-#3**)·`GET /api/evals/:loop/:stage/:run`(scorecard 상세).
...
- **제안 → 적용 = 서명 envelope + nonce + payload 일치(R1-#1·R1-#4·R2-#4·apply-state 계약):** 제안은 **F7 편집기 경로 + 사람 승인으로만** 반영. **자동 적용 절대 금지 · 제안 ≠ 적용 · Stage 4 자동 쓰기 없음.**
  - **서명 proposal envelope(R2-#4·R3-#5·R4-#2 — 가변 rollup-head 결속 제거):** 서버가 제안 생성 시 **canonical diff/내용 + 타깃 pathId + baseHash + 근거집합 digest + `effective-config 해시`**만 정확 결속해 **HMAC 서명**·해시. **가변 `rollup-head`는 envelope에 정확결속 안 함**(대기 중 새 run append 시 정확일치 실패로 정당 제안까지 브릭되던 모순 제거·신선도는 저장 시 게이트 재평가로 판정). 승인은 envelope + **일회용 nonce**를 F7에 전달(디스크 미변경·비-mutating).
  - **durable nonce(R3-#4):** nonce는 메모리 아님 **`<state_home>/evals-nonces`에 durable 저장**(envelope 해시+만료+state)·**issued→consumed 원자 전이(F7 mutation 前·atomic.ts 준용)**·소비 후 **tombstone 재시작 유지**(재시작 replay·대기 승인 유실 데드엔드 차단).
  - **F7이 독립 검증·저장(staleness 재평가·R3-#5·R4-#2):** 현재 정의 fetch → **정확한 diff 표시** → **에디터 주입 내용 == envelope canonical payload 정확 일치**(악성 payload 은밀 전달 차단) → **envelope `effective-config 해시`가 현재 config와 정확 일치 요구(config 완화 stale 차단) + A106 게이트를 현재(진행됐을 수 있는) rollup 상태로 재평가(fail-closed)**(rollup-head 정확일치 대신 재평가만 — 새 run append돼도 게이트 만족 시 도달 가능) → `definitionEditEnabled`·`pathId`/`baseHash` 강제(DW2/DW6)·**nonce 1회 소비** → **별도 명시 저장(PUT)**. **payload/diff 불일치·타깃 교체·config 해시 불일치·게이트 재평가 실패·nonce 재사용 = 거부(`409 stale-config`/`stale-evidence`)**.
...
| Part B 적용 | 자동 적용·payload 은밀 교체·replay·config staleness(제안↔승인 완화·R3-#5) | **자동 적용 금지·Stage 4 없음**·서명 envelope(**canonical payload+pathId+baseHash+effective-config 해시**·가변 rollup-head 미결속·R4-#2)·**durable nonce(tombstone·원자 소비)**·**F7 저장 시 config 해시 일치 + A106 현재 rollup 재평가(불일치 409 stale)**·payload 정확 일치·no-auto-apply(**tools·skills**) | F7·hmac.ts·atomic.ts |
...
| # | 기준 | M | 기능 |
|---|------|---|------|
| A47 | `GET /api/runs` 필터 확장: status+manifest **병합 스캔**(현 listRuns=status만 → manifest read 델타), 반환 `{runId,runtime,mode,state,recordedAt(FS-time·필터/정렬 축),createdAt(manifest·표시·괴리 가능),updatedAt,goal,agent,requestedBy}`. 무인자 호출 하위호환 | M7 | F4 |
...
| A50 | 열거·바운드(정렬정합+OOM+runId 형식 무의존·R3-#1·R4-#1): **이름만 readdir → `fs.stat` birthtimeMs(fallback mtimeMs) desc 정렬**(runId 형식 무관·UUID/`run-10`에도 정확)·**malformed/stat불가 quarantine** → 상위 `MAX_RUNS_SCAN`=5000만 `MAX_JSON_BYTES`=64KB 바운드 내용 read(OOM=내용 read 상한)·`SCAN_DEADLINE_MS`=2000·손상 `valid:false` skip·`MAX_RUN_DIRS` backstop·tie-break runId | M7 | F4 |
...
| A60 | `GET /api/metrics/{overview,agents,skills}`: 바운드 on-read 집계(F4 스캔 재사용·전수스캔 금지·supervisor/API 쓰기 없음=읽기전용) | M9 | F6 |
...
| A66 | 에이전트 실행=단일경로 POST /api/runs·서버 정의 재도출(클라 주장 무시)·manifest.agent 기록·삭제된 에이전트 태그는 형식검증만(무해) | M10 | F2 |
...
| # | 기준 | M | Part |
|---|------|---|------|
| A102 | Part A 소스 안전(R2-#1·R3-#1·R3-#3·R5·R7): **추세는 신뢰 `<state_home>/evals-rollup`(체인 검증)에서만**(`_workspace/summary.jsonl`=표시 소스 아님·"미검증")·**ingest = 서버 재도출-후-서명(판정 원장 `verdicts.json`서 집계 직접 재계산·`_workspace` precomputed `verdict_counts`/`alignment_score` 신뢰 안 함·재계산 불일치 격리·자기일관 위조 oracle 차단·R7)**·**이후 과거 scorecard 무결성=rollup digest 매칭(현재키 재검증 안 함→회전 무브릭·R5)**·`_workspace` 파일=표시용(digest 불일치=상세 "변조"·게이트 브릭 없음)·graceful(500 금지) | M13 | A |
...
| A107 | 제안→적용 = **서명 envelope+durable nonce+게이트 재평가(R2-#4·R3-#4·R3-#5·R4-#2):** envelope = **canonical diff/payload+pathId+baseHash+근거 digest+`effective-config 해시`** HMAC 서명(**가변 rollup-head 미결속·R4-#2**)·승인은 envelope+**durable nonce(`<state_home>/evals-nonces`·issued→consumed 원자·재시작 tombstone)**로 F7 열기만·**F7이 주입 payload==envelope 정확 일치 + config 해시 현재값 일치 + A106 현재 rollup 재평가(fail-closed·rollup-head 정확일치 아님)**·`definitionEditEnabled`·pathId/baseHash·nonce 1회 소비·별도 저장·**자동 적용 금지·Stage 4 쓰기 없음**·payload/타깃/config불일치/재평가실패/nonce재사용 → 거부(`409 stale`)·**no-auto-apply에 `tools`·`skills`**·audit·**UX(A112·UX-R1-#3): CTA="편집기에서 검토·저장"(승인 아님)·전환 前 "아직 적용되지 않음"·F7 저장 완료 전 "미적용" 유지(승인=반영 오인 방지)** | M13 | B |
...
| A110 | 저장 = **F3.7 확장(`evals`)·`.passthrough()`+per-leaf 독립 safeParse(전체/서브객체 `.strict()` 금지·`rollingN` 손상이 형제 `minAdjudicated:50` 리셋 안 함·R1-#6/R2-#5/R4-#4)·effective threshold=`max(설정값, floor)`(리셋돼도 floor 30/10/3 미만 불가)**·원자 RMW·뮤텍스·부재/손상→안전기본값 fail-closed·mutating 게이트(Origin/token). **UX(A112·UX-R1-#4): 입력 옆 최소 30/10/3 상시 표시·floor 미만 저장 전 인라인 거부(silent clamp 금지)·저장 전 old→new/effective diff·저장 후 적용값 피드백** | M13 | C |
...
| 스위트 | 케이스(전건 거부/차단) | 기준 |
|--------|------------------------|------|
| F4-쿼리 | `q=(a+)+`·`q=.*`(리터럴·ReDoS 없음)·`state=<임의문자열>`(400)·`limit=99999`(clamp)·`MAX_RUNS_SCAN` 초과(truncated)·**대량 디렉토리(이름 열거→FS시간 상위 N read)**·초과크기 manifest(skip)·**malformed/stat불가 runId(quarantine)**. **ACCEPT: UUID·`run-10` 형식 runId도 FS시간으로 최신 N 정확 선택** | A48·A49·A50·A51 |
...
| F2-에이전트 | `name=../foo`·`name=a b`(경로주입/메타)·**D 밖 allowedTools 주장(드롭·상향 불가)**·삭제된 에이전트 태그 | A64·A65·A66 |
...
| F8-Eval | 경로탈출·심링크·대용량·**미서명/서명불일치·수학 모순 scorecard(격리)**·**자기일관 위조 aggregate(precomputed `verdict_counts`/`alignment_score`→서버 원장 재계산 불일치 격리·R7)**·**스크립트 자가서명·키 재생성(서버 서명만·R3-#1)**·**scorecard XSS(→DV8)**·**게이트 우회: 29·9·null·flood·절단창·rollup 체인 링크 불일치/seq gap/head 서명 불일치/절단(fail-closed·R3-#2)**·**ACCEPT(브릭 0·R4-#1·R5·R6): 키 회전 후 과거 rollup 엔트리·과거 scorecard·활성 window `_workspace` 사후 변조 어느 쪽도 신규 제안 브릭 0(무결성=해시체인/rollup digest·게이트는 rollup만·gate-time `_workspace` 재읽기 없음)**·**_workspace 가짜 추세 UI 기만(rollup 소싱·R3-#3)**·**config<floor·형제 손상→floor 밑(per-leaf+max·R2-#5)**·**Stage 4 쓰기(400)**·**envelope payload 불일치/타깃 교체(가변 rollup-head 미결속·R4-#2)·config stale(409)·nonce in-session replay(durable·R3-#4)**·단계<3 제안·alignment 품질 오표시. **OUT-OF-SCOPE(게임오버·F8.8): `<state_home>` {rollup,head}/nonce 조율 rollback(cross-restart)** | A102·A105·A106·A107·A110·A111 |
**거부케이스 총 96+**(F4:7·F5:12·F2:4·F3:12+·F7:22+·**F8:38+**·state_home 장악 out-of-scope·ACCEPT 케이스 별도).
...
- **핵심 결정 & 이유:**
  ...
  - 스키마 변경 없음(F1 델타 폐기) — v0.5 정본 §5 그대로. `manifest.agent`(F2)·events `usage`(F6)만 소비(선반영분).
...
- **다음 단계:** 보안 R1-R4 + **UX 레이어(A81-A101·UX 수렴 완료)** + **F8(A102-A112·보안 R1~R7 완전 수렴·재도출-후-서명·정직 스코핑 F8.8·게이트/표시 분리·사람 승인 backstop)** → **UX 외부감사** → M7(F4)→M8(F5)→M9(F6)→M10(F2)→M11(F3)→M12(F7)→**M13(F8·평가·마지막)** 순차 구현(각 마일스톤 DoD = 기능 A + UX A81-A101 횡단). 전체 A47-A112(기능 34+UX 21+F8 11)·거부 96+·state_home 장악·self-scoring 참값화 out-of-scope 문서화.
</content>
</invoke>
