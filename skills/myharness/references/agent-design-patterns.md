...
## 목차 (필요한 섹션만 로드)
- 실행 모드(팀 vs 서브) — 모드 선택 시 · 아키텍처 유형(6패턴) — 패턴 선택 시 · 복합 패턴 — Phase별 혼합 시
- 에이전트 타입 선택 / 정의 구조 — Phase 3 생성 시 · 분리 기준 / 재사용 설계 — 3-0 중복 검토 시 · 스킬↔에이전트 — 연결 결정 시
...
| **적대적 의사결정 검토** (팬아웃 + 반복 생성-검증) | 입장별 병렬 주장 → 상대 주장 주입 → 반박 라운드 → 심판(오케스트레이터) 판정. 별도 1급 패턴 아님 — 기존 조합 + 루프 제어 | 아키텍처 찬반·기술 채택 결정. **단** 같은 엔진 논객은 같은 맹점 공유(가짜 토론) → 진짜 독립은 다엔진이라 사실상 `external-review-loop`의 의사결정 응용. 명백한 사안엔 쓰지 말 것(false balance). 라운드마다 컨텍스트 누적 = 토큰 팽창 → 중대 결정만, 루프는 오케스트레이터가 제어 |
...
| 단순 조사/수집이고 프롬프트만으로 충분 | **`general-purpose`** + 상세 프롬프트 | 빌트인 타입을 쓰되 정의 파일은 아래 원칙대로 생성(역할·원칙을 파일로) |
